...
class RestGateway extends AbstractGateway
{
    ...
    public function getPaymentViewName($type = null)
    {
        if ($type == "subscription") {
            return "PayPal::subscription-checkout";

        } else if ($type == "ecommerce") {
            return "PayPal::ecommerce-checkout";
    ...
    public function preparePaymentTokenParameters(Order $order)
    {
        $parameters = [
            'amount' => \Payments::currency_convert($order->amount, null, $order->currency, false),
            'currency' => $order->currency,
            'description' => "Payment for Order# : {$order->order_number}",
            'returnUrl' => url('/payment/paypal/return/?txn_id=' . $order->order_number),
            'cancelUrl' => url('/payment/paypal/return/?txn_id=' . $order->order_number),
        ];
    ...
    public function prepareCreateChargeParameters(Order $order, User $user, $checkoutDetails)
    {
        return [
